...
async function getDataByDate(req, collectionName, foods, date) {
  ...
  if (date.length === 4) {  // Jika hanya tahun yang diberikan (YYYY)
    dateStart = `${date}-01-01:00.00`;  // Mulai dari 1 Januari jam 00:00
    dateEnd = `${date}-12-31:23.59`;    // Hingga 31 Desember jam 23:59
  } 
  else if (date.length === 7) {  // Jika tahun dan bulan diberikan (YYYY-MM)
  ...
  else if (date.length === 10) {  // Jika tahun, bulan, dan hari diberikan (YYYY-MM-DD)
