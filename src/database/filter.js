...
const getDataByMonth = async (userID, month) => {
    const startDate = moment().month(month - 1).startOf('month').format('DD-MM-YYYY');
    const endDate = moment().month(month - 1).endOf('month').format('DD-MM-YYYY');
    console.log(startDate);
    console.log(endDate);
    
    const userCollection = db.collection(userID);
    const snapshot = await userCollection.where('date', '>=', startDate).where('date', '<=', endDate).get();
  ...
  const getDataByWeek = async (userID, month, week) => {
    const startOfMonth = moment().month(month - 1).startOf('month');
  ...
  module.exports = { getDataByYear, getDataByMonth, getDataByDate, getDataByWeek }